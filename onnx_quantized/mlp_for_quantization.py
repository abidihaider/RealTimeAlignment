...
class SubsetSolver(nn.Module):
    ...
    @staticmethod
    def roll(data, shifts):
        """
        Use slicing and concatenation to simulate rolling.
        """
        
        return torch.cat([data[:, -shifts:], data[:, :-shifts]], dim=1)

    def _assemble_shift(self, data):
        """
        Modified for Quantization
        Quantized model cannot have rolling operation.
        We will use slicing and concatenation to replace rolling.
        """
        
        return torch.cat([self.roll(data, shifts=i)
