...
from rtal.utils import (get_lr,
                        count_parameters,
                        get_data_root)
from rtal.utils import CheckpointerQAT
from rtal.utils import Cumulator
...
def run_epoch(model,
    ...
    for event in pbar:
        ...
        center_curr = detector_curr[..., 0:3]
        # u,v_curr shape = (batch_size, num_detectors, 3)
        # u,v_curr is the global 3D location of the current
        # u and v axes
        u_curr = detector_curr[..., 3:6]
        v_curr = detector_curr[..., 6:]

        detector_start = event['detector_start'].to(device)
        center_start = detector_curr[..., 0:3]

        # center_misaligment shape = (batch_size, num_detectors, 3)
        center_misalignment = center_curr - center_start

        # u,v_ref_coords and rho shape = (batch_size, num_detectors)
        # u/v_ref_coords[b, d] is the u/v coordinate of the current
        # misaligned normal in the aligned (correct) coordinate system.
        # rho[b, d] is the rotation of the misaligned (u, v)-plane.
        # u,v_ref_coords and rho are called frame misalignment.
        u_ref_coord, v_ref_coord, rho = mal.get_misalignment(u_curr, v_curr)
        # frame_misaligment shape = (batch_size, num_detectors, 3)
        frame_misalignment = torch.cat([u_ref_coord.unsqueeze(-1),
        ...
        # run the model, calculate loss(es), and update network
        if is_train:
            misalignment_pred = model(readout.flatten(-2, -1))
        else:
            misalignment_pred = model.inference(readout.flatten(-2, -1), randperm=False)

        misalignment_pred = misalignment_pred.reshape(-1, 3, 6)
        center_misalignment_pred = misalignment_pred[..., :3]
        frame_misalignment_pred = misalignment_pred[..., 3:]

        center_loss = loss_fn(center_misalignment_pred, center_misalignment)
        frame_loss = loss_fn(frame_misalignment_pred, frame_misalignment)
...
def train():
    ...
    # Checkpointer
    checkpointer = CheckpointerQAT(model,
                                   optimizer       = optimizer,
                                   scheduler       = scheduler,
    ...
    for epoch in range(resume_epoch + 1, num_epochs + 1):
        ...
        # save checkpoints
        checkpointer.save(epoch, save_converted=True)
